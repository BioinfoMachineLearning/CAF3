...
class HHRParser:
    ...
    def parse_hhr_file(self, hhr_file: str) -> List[Dict]:
        ...
        lines = content.split('\n')
        print("DEBUG: First 20 lines of HHR file:")
        # for i, line in enumerate(lines[:20]):
        #     print(f"{i+1:2d}: {line}")
        
        # Split by template hits (each starts with "> " or "No ")
        # First try splitting by "> "
        template_blocks = re.split(r'\n>', content)
        
        if len(template_blocks) <= 1:
            # Try alternative splitting pattern - some HHR files use different formats
            template_blocks = re.split(r'\nNo\s+\d+', content)
        
        print(f"DEBUG: Found {len(template_blocks)} potential template blocks")
        ...
        for i, block in enumerate(template_blocks):
            ...
            print(f"DEBUG: Block starts with: {block[:100]}")
            
            template_data = self._parse_template_block(block)
            if template_data:
                templates.append(template_data)
                print(f"DEBUG: Successfully parsed template: {template_data['template_name']}")
            else:
        ...
        while i < len(alignment_lines):
            ...
            if line.startswith('Q ') and len(line.split()) >= 4:
                ...
                if template_line:
                    ...
                    print("Here__________ ",template_parts)
    ...
    def _extract_aligned_positions(self, query_seq: str, template_seq: str, 
        ...
        # print("Printing:",query_seq,template_seq)
...
def run_hhr_parser(
    ...
    return 0
# def main():
#     parser = argparse.ArgumentParser(description='Parse HHR files and extract template alignments')
#     parser.add_argument('hhr_file', help='Input HHR file')
#     parser.add_argument('-o', '--output', default='template_alignments.csv', 
#                        help='Output CSV file (default: template_alignments.csv)')
#     parser.add_argument('--detailed', action='store_true',
#                        help='Save detailed format (one alignment pair per row)')
...
#     # Check if input file exists
#     if not Path(args.hhr_file).exists():
#         print(f"Error: HHR file '{args.hhr_file}' not found.")
#         return 1
...
#     if args.verbose:
#         print(f"Parsing HHR file: {args.hhr_file}")
    
#     templates = hhr_parser.parse_hhr_file(args.hhr_file)
...
#     # Save to CSV
#     if args.detailed:
#         hhr_parser.save_to_detailed_csv(templates, args.output)
#     else:
#         hhr_parser.save_to_csv(templates, args.output)
    
#     print(f"Template alignments saved to: {args.output}")
#     return 0
